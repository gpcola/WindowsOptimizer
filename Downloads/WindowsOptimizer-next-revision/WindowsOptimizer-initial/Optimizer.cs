...
if (-not $pkgs) {{
    ...
    foreach ($pkg in $pkgs) {{
        ...
        {
            log("Clearing update cache and delivery optimization cache...");
            bool ok1 = ExecuteStandard("Stop-Service wuauserv -Force -ErrorAction SilentlyContinue");
            bool ok2 = ExecuteStandard("Stop-Service bits -Force -ErrorAction SilentlyContinue");
            bool ok3 = ExecuteStandard(@"Remove-Item 'C:\Windows\SoftwareDistribution\Download\*' -Recurse -Force -ErrorAction SilentlyContinue");
            bool ok4 = ExecuteStandard(@"Remove-Item 'C:\Windows\DeliveryOptimization\*' -Recurse -Force -ErrorAction SilentlyContinue");
            bool ok5 = ExecuteStandard("Start-Service bits -ErrorAction SilentlyContinue");
            bool ok6 = ExecuteStandard("Start-Service wuauserv -ErrorAction SilentlyContinue");
